...
predict_fun_ci <- function(age = 60,
  ...
  # Read in the model
  mod <- readRDS("bart_model_reduced_20191218.rds")
  ...
  full_post <- bart_machine_get_posterior(mod, new_x)$y_hat_posterior_samples
  p_model_all <- 1 - apply(full_post, 1, quantile, probs = rev(levels))
